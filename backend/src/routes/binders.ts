...
export const createBinderRouter = (binderService: BinderService) => {
  ...
  router.post(
    ...
    asyncHandler(async (req: AuthRequest, res: Response) => {
      const db = getDb();
      const plan = await generateBinderPlan(db, req.user?.id ?? 0, req.body.prompt, {
        ...
        maxSingleCardPrice:
          req.body.maxSingleCardPrice != null
  ...
  router.post(
    '/',
    authenticate,
    validate(createBinderSchema),
    asyncHandler(async (req: AuthRequest, res: Response) => {
      const slots = req.body.slots?.map(
        (s: {
          pageNumber: number;
          slotPosition: number;
          cardId: string;
  ...
  router.post(
    ...
    asyncHandler(async (req: AuthRequest, res: Response) => {
      ...
      for (let i = 0; i < plan.slots.length; i++) {
        ...
        const existingSlot = (await binderService.getBinderWithSlots(binderId, req.user!.id))
          ?.slots[i];
        if (existingSlot) {
