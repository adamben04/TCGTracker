...
const normalizeRarity = (rarity?: string | null): string => {
  ...
  if (r.includes('secret') || r.includes('special') || r.includes('illustration') || r.includes('gold') || r.includes('rainbow')) return 'special';
  if (r.includes('hyper') || r.includes('full art') || r.includes('ultra') || r.includes('vmax') || r.includes('vstar')) return 'ultra';
  if (r.includes('holo') || r.includes('radiant') || r.includes('shiny') || r.includes('v ')) return 'holo';
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const mappingRows = await allDbRows<{ uniqueIdentifier: string; cardName: string; rarity: string | null }>(
        db,
      ...
      void (async () => {
        const pending = chaseRows
          .filter((c) => c.psa10Price == null)
          .slice(0, 10);
        if (pending.length === 0) return;
        try {
          for (const card of pending) {
            await getGradedPrices(
              card.cardId,
              card.cardName,
              String(setId),
              setRow.setName
