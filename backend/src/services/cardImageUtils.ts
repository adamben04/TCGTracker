...
export const buildPlaceholderImage = (name: string, set: string) =>
  ...
  '%3C/text%3E%3Ctext x="50%25" y="55%25" font-family="Arial,sans-serif" font-size="14" fill="%23d1d5db" text-anchor="middle"%3E' +
  encodeURIComponent(set) +
  '%3C/text%3E%3Ctext x="50%25" y="65%25" font-family="Arial,sans-serif" font-size="12" fill="%23e5e7eb" text-anchor="middle"%3ENo Image%3C/text%3E%3C/svg%3E';

export const buildDeterministicImageUrls = async (
  setId?: string | null,
...
export const hasImageMetadataColumns = async (): Promise<boolean> => {
  if (imageColumnCache && Date.now() - imageColumnCache.checkedAt < IMAGE_COLUMN_CACHE_TTL) {
