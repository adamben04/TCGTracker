...
export const InvestmentBadge: React.FC<InvestmentBadgeProps> = ({
  investmentData,
  compact = false,
}) => {
  const getRecommendationColor = (rec: string) => {
    switch (rec) {
      case 'BUY':
        return 'bg-gain-muted text-gain border-gain/25';
      case 'HOLD':
        return 'bg-accent-muted text-accent border-accent/25';
      case 'SELL':
        return 'bg-loss-muted text-loss border-loss/25';
      case 'WATCH':
        return 'bg-surface-hover text-ink-secondary border-border-default';
      default:
        return 'bg-surface-inset text-ink-muted border-border-default';
    }
  };
  ...
  if (compact) {
    return (
      <div className="flex gap-1 flex-wrap">
        <span
          className={`px-2 py-1 rounded-full text-xs font-medium border ${getRecommendationColor(investmentData.recommendation)}`}
        >
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        <span
          className={`px-3 py-1 rounded-full text-sm font-medium border ${getRecommendationColor(investmentData.recommendation)}`}
        >
          {investmentData.recommendation}
        </span>
      ...
      <div className="grid grid-cols-2 gap-2 text-xs">
        <div className="flex items-center gap-1">
          ...
          <span className="font-medium text-ink-primary">
            {investmentData.marketAnalysis.trend}
          </span>
        </div>
        <div className="flex items-center gap-1">
          <AlertTriangle
            className={`w-3 h-3 ${getRiskColor(investmentData.riskLevel)}`}
            aria-hidden="true"
          />
      ...
      {(investmentData.marketAnalysis.isUndervalued ||
        ...
        <div className="text-xs">
          {investmentData.marketAnalysis.isUndervalued && (
            <span className="px-2 py-1 bg-gain-muted text-gain rounded-full">Undervalued</span>
          )}
          {investmentData.marketAnalysis.isOvervalued && (
            <span className="px-2 py-1 bg-loss-muted text-loss rounded-full">Overvalued</span>
          )}
        </div>
      )}
    </div>
